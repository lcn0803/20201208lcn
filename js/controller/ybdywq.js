angular.module('controller_ybdywq', ['ionic', 'highcharts-ng', 'ngCordova', 'services'])

//待签约案例查询
.controller('ybdywqDqalCxController',function($rootScope,$scope,$state,$http,MyFactoryNew){
	var bdcqzh = "";
	var syqrmc = "";
    ...
    $scope.search = function(){
    	...
    	$state.go('ybdywq_dqal_list');
    }
})
//一般抵押网签待签案例-列表页面
.controller('ybdyWqDqalListController',function($rootScope,$scope,$http,$state,$ionicPopup,$interval,MyFactoryNew){
	...
	if(p.test(pdqzORht)){
		...
		$scope.getDbal = function() {
			...
			MyFactoryNew.getHttpRequest( baseurl + '/bitsroad/queryPageJsonWithRB', 'POST', null, requestBody).then(function (data) {
				if (data && data._page && data._page.count > 1){
					...
					MyFactoryNew.listHtmlInitAndGoto(data, 'ybdywq_dqalList', 'ybdywq_dqal_list');
					...
					$scope.clfqyOrysdj=true;
					$state.go('ybdywq_dqal_detail',{bdcqzh:bdcqzh,syqrmc:syqrmc});
		...
		$scope.doRefresh= function(ifFromPullRefresh) {
			...
			MyFactoryNew.loadMore('/bitsroad/queryPageJsonWithRB',requestBody,"ybdywq_dqalList",true,ifFromPullRefresh);
		}
		
		$scope.loadMore= function(listName) {
	        var requestBody = {_module:'TT',_method:'ybdywq_getclfOrysdjDqalListByPage',bdcqzh:bdcqzh,syqrmc:syqrmc};
			MyFactoryNew.loadMore('/bitsroad/queryPageJsonWithRB',requestBody,"ybdywq_dqalList",false);
		}
	}else{
		//待办案例list页面
		$scope.getDbal = function() {
...
//一般抵押网签待签案例-详细信息
.controller('ybdyWqDqalDetailController',function($rootScope,$scope,$http,$state,$stateParams,$ionicPopup,$interval,$ionicPopover,MyFactoryNew){
	...
	if(ismrhavesign==0){
		...
		$scope.searchPopup = $ionicPopup.show({
    		templateUrl:baseurl+'/static/tt/htm/public/ybdywqPop.html',
			title:'温馨提示',
			scope: $rootScope,
...
//一般抵押网签-抵押网签-列表页面
.controller('ybdyWqDywqListController',function($rootScope,$scope,$state,$ionicPopup,$ionicListDelegate, $http, $ionicLoading,$ionicModal,$cordovaCamera,$ionicPopover,MyFactoryNew){
	var listName = "ybdywq_dywqList";
	//图片编辑查看页面
	$scope.goEdit = function(id,mastatus){
		...
		$state.go('editPics',{id:id,backURL:'ybdywq_dywq_list'});
	};

	//刷新
 	$scope.doRefresh= function(ifFromPullRefresh) {
        var requestBody = {_module:'TT',_method:'ybdywq_getDywqListByPage'};
    ...
    $scope.downPDF= function(ywbh) {
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.downPDF(140,paramsObj);
    }
    ...
    $scope.creQRCode = function(ywbh){
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.showPdfQRCode(140,paramsObj,"ybdywq_dywq_list");
    } 	
...
//一般抵押网签业务详细查看或保存---多个页签页面
.controller('ybdyWqDywqDetailController', function($rootScope,$scope,$http,$state,$ionicPopup,$interval,$stateParams,MyFactoryNew){
	...
	function isDisabled(){
		...
		if(bastatus == 0 && mastatus == 1619040 && g_getRoleno() != PUBLIC_JS_GLY){
	...
	$scope.zwrxxxx = function ($event,ng_obj) {
		...
		$rootScope.goRouteForBack("pubZwrxx",{id:$stateParams.id,qlrxh:qlrxh,dwgr:dwgr,backURL:'ybdywq_detail',if_cz:$scope.if_show_baocunButton},{id:$stateParams.id});
	}
    //增加债务人
