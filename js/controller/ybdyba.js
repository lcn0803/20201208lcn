angular.module('controller_ybdyba', ['ionic', 'highcharts-ng', 'ngCordova', 'services'])

//一般抵押备案待办案例-列表页面
.controller('ybdybaDbalListController',function($rootScope,$scope,$http,$state,$ionicPopup,$interval,MyFactoryNew){
	var xdywValue = $rootScope._app_ybdyba_xdyw;
	//待办案例list页面
	$scope.getDbal = function() {
		//交互获得案例数据源list
	    var requestBody = {_module:'TT',_method:'ybdyba_getDbalListByPage',value:xdywValue,pageNum:1};
		MyFactoryNew.getHttpRequest( baseurl + '/bitsroad/queryPageJsonWithRB', 'POST', null, requestBody).then(function (data) {
			if (data && data._page && data._page.count > 1){
				MyFactoryNew.listHtmlInitAndGoto(data, 'ybdyba_dbalList', 'ybdyba_dbal_list');
			}else if(data && data._page && data._page.count == 1){
				var id = data._page.results[0].YWBH;
				$rootScope.ybdyba_dbalList = data._page.results;
				$rootScope.dbal = data._page.results[0];
				$state.go('ybdyba_dbal_detail');
	...
	$scope.doRefresh= function(ifFromPullRefresh) {
        var requestBody = {_module:'TT',_method:'ybdyba_getDbalListByPage',value:xdywValue};
		MyFactoryNew.loadMore('/bitsroad/queryPageJsonWithRB',requestBody,"ybdyba_dbalList",true,ifFromPullRefresh);
	}
	
	$scope.loadMore= function(listName) {
        var requestBody = {_module:'TT',_method:'ybdyba_getDbalListByPage',value:xdywValue};
		MyFactoryNew.loadMore('/bitsroad/queryPageJsonWithRB',requestBody,"ybdyba_dbalList",false);
	}
	
})	

//一般抵押备案待办案例-详细信息
.controller('ybdybaDbalDetailController',function($rootScope,$scope,$http,$state,$ionicPopup,$interval,$ionicPopover,MyFactoryNew){
	//取消
...
//一般抵押备案-交易备案-列表页面
.controller('ybdybaJybaListController',function($rootScope,$scope,$state,$ionicPopup,$ionicListDelegate, $http, $ionicLoading,$ionicModal,$cordovaCamera,$ionicPopover,MyFactoryNew){
	var xdywValue = $rootScope._app_ybdyba_xdyw;
	var listName = "ybdyba_jybaList";
	//图片编辑查看页面
	$scope.goEdit = function(id,mastatus){
		...
		$state.go('editPics',{id:id,backURL:'ybdyba_jyba_list'});
	};

	//刷新
 	$scope.doRefresh= function(ifFromPullRefresh) {
        var requestBody = {_module:'TT',_method:'ybdyba_getJybaListByPage',value:xdywValue};
    ...
    $scope.downPDF= function(ywbh) {
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.downPDF(37,paramsObj);
    }
    ...
    $scope.showPDFImg= function(ywbh) {
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.showPDFImg(37,paramsObj);		
    }
    ...
    $scope.creQRCode = function(ywbh){
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.showPdfQRCode(37,paramsObj,"ybdyba_jyba_list");
    } 	
    ...
    $scope.downPDF_wq= function(ywbh) {
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.downPDF(141,paramsObj);
    }
    ...
    $scope.creQRCode_wq = function(ywbh){
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.showPdfQRCode(141,paramsObj,"ybdyba_jyba_list");
    }    
	...
	$scope.tj= function(id,flowid,businessID,stanID,realTypeID,svalue) {
 	...
 	$scope.del = function(id) {
        var requestBody = {_module:'TT',_method:'ybdyba_delJyba',id:id};
 		MyFactoryNew.getHttpRequest( baseurl + '/bitsroad/rpcjsonWithRB', 'POST', null, requestBody).then(function (data) {
 			if(data && data.type && data.type=="success"){
 				$rootScope.mttAlert(PUBLIC_OPER_SUCCESS,function() {
 					$scope.doRefresh();
...
//业务详细查看或保存---多个页签页面
.controller('ybdybaJybaDetailController', function($rootScope,$scope,$http,$state,$ionicPopup,$interval,$stateParams,MyFactoryNew){
	...
	function isDisabled(){
		...
		if(bastatus == 0 && mastatus == 1620040 && parentBaseactivityid && g_getRoleno() != PUBLIC_JS_GLY){//当父类业务大类为【存量房网签－导入】时，六个字段可编写录入，备注可修改
		...
		}else if(bastatus == 0 && mastatus == 1620040 && !parentBaseactivityid && g_getRoleno() != PUBLIC_JS_GLY){//当父类业务大类为【一般抵押签约新建】时，只有备注可修改
			$scope.if_show_baocunButton = true;
			$scope.yw_dywq = true;
			$(".ifEdit").removeAttr("disabled");
			$("textarea.ifEdit").css("background","white");
	...
	$scope.save_yw_2 = function(){
		...
		var requestBody = {_module:'TT',_method:'saveYbdybaYwInfo',id:id,description:description,dklb:dklb,dyhth:dyhth,dbfw:dbfw,bdbzqse:bdbzqse,zwlxqsrq:zwlxqsrq,zwlxzzrq:zwlxzzrq};
		MyFactoryNew.getHttpRequest( baseurl + '/bitsroad/rpcjsonWithRB', 'POST', null, requestBody).then(function (data) {
			if(data && data.type && data.type=="success"){
				$rootScope.mttAlert(PUBLIC_OPER_SUCCESS);
			}else if(data && data.type && data.type=="error"){
