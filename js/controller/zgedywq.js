angular.module('controller_zgedywq', ['ionic', 'highcharts-ng', 'ngCordova', 'services'])

//待签约案例查询
.controller('zgedywqDqalCxController',function($rootScope,$scope,$state,$http,MyFactoryNew){
	var bdcqzh = "";
	var syqrmc = "";
    ...
    $scope.search = function(){
    	...
    	$state.go('zgedywq_dqal_list');
    }
})
//最高额抵押网签待签案例-列表页面
.controller('zgedyWqDqalListController',function($rootScope,$scope,$http,$state,$ionicPopup,$interval,MyFactoryNew){
	
	var bdcqzh = $rootScope.dqfw_bdcqzh;
	var syqrmc = $rootScope.dqfw_syqrmc;
	var p=/^[a-zA-Z]/;
	var pdqzORht=bdcqzh.substring(0,1);
	
	if(p.test(pdqzORht)){
		//alert(pdqzORht+"->进入存量房签约和预售登记案例");
		//待办案例list页面
		$scope.getDbal = function() {
			//交互获得案例数据源list
		    var requestBody = {_module:'TT',_method:'ybdywq_getclfOrysdjDqalListByPage',bdcqzh:bdcqzh,syqrmc:syqrmc,pageNum:1};
			MyFactoryNew.getHttpRequest( baseurl + '/bitsroad/queryPageJsonWithRB', 'POST', null, requestBody).then(function (data) {
				if (data && data._page && data._page.count > 1){
					$scope.clfqyOrysdj=true;
					MyFactoryNew.listHtmlInitAndGoto(data, 'zgedywq_dqalList', 'zgedywq_dqal_list');
				}else if(data && data._page && data._page.count == 1){
					var id = data._page.results[0].YWBH;
					$rootScope.zgedywq_dqalList = data._page.results;
					$rootScope.dbal = data._page.results[0];
					$scope.clfqyOrysdj=true;
					$state.go('zgedywq_dqal_detail',{bdcqzh:bdcqzh,syqrmc:syqrmc});
		...
		$scope.goDetailHtml = function(dbalObject) {
			if(dbalObject){
				...
				$rootScope.mttAlert('获取待签案例详细信息失败!')
			}
		}
		
		$scope.doRefresh= function(ifFromPullRefresh) {
	        var requestBody = {_module:'TT',_method:'ybdywq_getclfOrysdjDqalListByPage',bdcqzh:bdcqzh,syqrmc:syqrmc};
			MyFactoryNew.loadMore('/bitsroad/queryPageJsonWithRB',requestBody,"zgedywq_dqalList",true,ifFromPullRefresh);
		}
		
		$scope.loadMore= function(listName) {
	        var requestBody = {_module:'TT',_method:'ybdywq_getclfOrysdjDqalListByPage',bdcqzh:bdcqzh,syqrmc:syqrmc};
			MyFactoryNew.loadMore('/bitsroad/queryPageJsonWithRB',requestBody,"zgedywq_dqalList",false);
		}
	}else{
		//alert(pdqzORht+"->进入不动产权证号案例");
		//待办案例list页面
		$scope.getDbal = function() {
			//交互获得案例数据源list
		    var requestBody = {_module:'TT',_method:'ybdywq_getbdcqzhDqalListByPage',bdcqzh:bdcqzh,syqrmc:syqrmc,pageNum:1};
			MyFactoryNew.getHttpRequest( baseurl + '/bitsroad/queryPageJsonWithRB', 'POST', null, requestBody).then(function (data) {
				if (data && data._page && data._page.count > 1){
					$scope.bdcqzhdr=true;
					MyFactoryNew.listHtmlInitAndGoto(data, 'zgedywq_dqalList', 'zgedywq_dqal_list');
					...
					var id = data._page.results[0].YWBH;
					$rootScope.ybdywq_dqalList = data._page.results;
					$rootScope.dbal = data._page.results[0];
					$scope.bdcqzhdr=true;
					$state.go('zgedywq_dqal_detail',{bdcqzh:bdcqzh,syqrmc:syqrmc});
				}else if (data && data._page && data._page.count == 0){
		...
		$scope.doRefresh= function(ifFromPullRefresh) {
	        var requestBody = {_module:'TT',_method:'ybdywq_getbdcqzhDqalListByPage',bdcqzh:bdcqzh,syqrmc:syqrmc};
			MyFactoryNew.loadMore('/bitsroad/queryPageJsonWithRB',requestBody,"zgedywq_dqalList",true,ifFromPullRefresh);
		}
...
//最高额抵押网签待签案例-详细信息
.controller('zgedyWqDqalDetailController',function($rootScope,$scope,$http,$state,$stateParams,$ionicPopup,$interval,$ionicPopover,MyFactoryNew){
	
	var bdcqzh = $stateParams.bdcqzh;
	var syqrmc = $stateParams.syqrmc;
    
    var bdcq = $rootScope.dqfw_bdcqzh;
    var p=/^[a-zA-Z]/;
	var pdqz=bdcq.substring(0,1);
	if(p.test(bdcq)){
		$scope.clfqyOrysdj=true;
	}else{
		$scope.bdcqzhdr=true;
	...
	var ishmrhavesign = $scope.dbal.ISHMRHAVESIGN;
	if(ishmrhavesign==0){
		$scope.banli = function() {
			var id = $scope.dbal.YWBH;
	        var requestBody = {_module:'TT',_method:'zgedywq_submitDqal',bdcqzh:bdcqzh,syqrmc:syqrmc};
			MyFactoryNew.getHttpRequest( baseurl + '/bitsroad/rpcjsonWithRB', 'POST', null, requestBody).then(function (data) {
				if(data && data.type && data.type=="success"){
					$state.go('zgedywq_dywq_list');
		...
		$scope.searchPopup = $ionicPopup.show({
    		templateUrl:baseurl+'/static/tt/htm/public/zgedywqPop.html',
			title:'温馨提示',
			scope: $rootScope,
...
//最高额抵押网签-抵押网签-列表页面
.controller('zgedyWqDywqListController',function($rootScope,$scope,$state,$ionicPopup,$ionicListDelegate, $http, $ionicLoading,$ionicModal,$cordovaCamera,$ionicPopover,MyFactoryNew){

	var listName = "zgedywq_dywqList";
	//图片编辑查看页面
	$scope.goEdit = function(id,mastatus){
		...
		$state.go('editPics',{id:id,backURL:'zgedywq_dywq_list'});
	};

	//刷新
 	$scope.doRefresh= function(ifFromPullRefresh) {
        var requestBody = {_module:'TT',_method:'zgedywq_getDywqListByPage'};
    ...
    $scope.downPDF= function(ywbh) {
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.downPDF(170,paramsObj);
    }
    ...
    $scope.creQRCode = function(ywbh){
    	var paramsObj ={};
    	paramsObj.ywbh = ywbh;
    	MyFactoryNew.showPdfQRCode(170,paramsObj,"zgedywq_dywq_list");
    } 	
...
//最高额抵押网签业务详细查看或保存---多个页签页面
.controller('zgedyWqDywqDetailController', function($rootScope,$scope,$http,$state,$ionicPopup,$interval,$stateParams,MyFactoryNew){
	...
	function isDisabled(){
		...
		if(bastatus == 0 && mastatus == 1639040 && g_getRoleno() != PUBLIC_JS_GLY ){
	...
	$scope.zwrxxxx = function ($event,ng_obj) {
		...
		$rootScope.goRouteForBack("pubZwrxx",{id:$stateParams.id,qlrxh:qlrxh,dwgr:dwgr,backURL:'zgedywq_detail',if_cz:$scope.if_show_baocunButton},{id:$stateParams.id});
	}
    //增加债务人
	...
	$scope.save_yw = function(){
		...
		var requestBody = {_module:'TT',_method:'ggsavedyWqYwxx',id:id,description:description,pgjz:pgjz,dklb:dklb,dyhth:dyhth,dbfw:dbfw,bdbzqse:bdbzqse,zwlxqsrq:zwlxqsrq,zwlxzzrq:zwlxzzrq};
		MyFactoryNew.getHttpRequest( baseurl + '/bitsroad/rpcjsonWithRB', 'POST', null, requestBody).then(function (data) {
			if(data && data.type && data.type=="success"){
				$rootScope.mttAlert(PUBLIC_OPER_SUCCESS);
			}else if(data && data.type && data.type=="error"){
